...
public class Main {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < amountBill; i++) {
			System.out.printf("%s %s %s %s %s %d %d %d %d", Bil[i].ID, Bil[i].c.name, Bil[i].c.address, Bil[i].m.name
					,Bil[i].m.unit, Bil[i].m.buyPrice, Bil[i].m.sellPrice, Bil[i].amount, Bil[i].balance);
			System.out.println();
		}
		input.close();
	}
